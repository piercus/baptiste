...
/**
* Example:
*   IoC.use(module.dir({prefix: 'myModule'}));
*
* @param {object} opts
...
Baptiste.prototype.dir = function (opts) {
	let prefix;
	let reg;
	if(opts && opts.prefix){
		reg = new RegExp('^'+opts.prefix+'(.*)');
	}
	...
	return function app(id){
		...
		for(var i = self.paths.length -1; i >= 0; i--){
			const comp = electrolyte.dir(Object.assign({}, opts, {dirname: self.paths[i]}))(id2);
